...
exports.compose = compose;(...presetNames) { return presetNames.filter(Boolean); }
exports.resolveStylePresets = resolveStylePresets;(keys = [], styleVars = {}) {
...
exports.getThemeVars = getThemeVars;(theme = 'light') { return themeVars[theme]; }
exports.getJSPreset = getJSPreset;(name) { return jsPresets[name]; }
exports.getHTMLPreset = getHTMLPreset;(name) { return htmlPresets[name]; }
exports.listPresetStats = listPresetStats;() { return { style: Object.keys(stylePresets).length, js: Object.keys(jsPresets).length, html: Object.keys(htmlPresets).length, cache: styleCache.size }; }
exports.createTailwindJITLayer = createTailwindJITLayer;(classes) { return classes.filter(Boolean).join(' '); }

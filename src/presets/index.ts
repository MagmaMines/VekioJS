...
const jsPresets: Record<string, (...args: any[]) => any> = {};
const htmlPresets: Record<string, (opts?: Record<string, any>) => any> = {};
...
baseOps.forEach((base, idx) => {
  for (let i = 1; i <= 30; i++) jsPresets[`${base}_${i}`] = (...args: any[]) => ({ op: base, variant: i, args, idx });
});
...
export function compose(...presetNames: string[]) { return presetNames.filter(Boolean); }

export function resolveStylePresets(keys: string[] = [], styleVars: Record<string, string> = {}) {
  const cacheKey = JSON.stringify([keys, styleVars]);
  const hit = styleCache.get(cacheKey);
  if (hit) return hit;
  const merged = keys.reduce((acc, k) => Object.assign(acc, stylePresets[k] || {}), {} as Record<string, any>);
  Object.keys(styleVars).forEach(k => { merged[k] = `var(${k})`; });
  styleCache.set(cacheKey, merged);
  return merged;
}

export function getThemeVars(theme: 'light'|'dark'|'system'='light'){ return themeVars[theme]; }
export function getJSPreset(name: string) { return jsPresets[name]; }
export function getHTMLPreset(name: string) { return htmlPresets[name]; }
...
export function createTailwindJITLayer(classes: string[]) { return classes.filter(Boolean).join(' '); }
